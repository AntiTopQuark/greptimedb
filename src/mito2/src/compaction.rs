...
mod tests {
    ...
    async fn test_find_compaction_options_priority() {
        fn schema_value_with_twcs(time_window: &str) -> SchemaNameValue {
            ...
            schema_value
                .extra_options
                .insert("compaction.twcs.time_window".to_string(), time_window.to_string());
            schema_value
        }
        ...
        for (case_name, schema_value, override_set, table_window, expected_window) in cases {
            ...
            if let Some(window) = table_window {
                if let crate::region::options::CompactionOptions::Twcs(twcs) =
            ...
            let (opts, _) =
                find_dynamic_options(table_id, &region_opts, &schema_metadata_manager)
                    .await
                    .unwrap();
            match opts {
                crate::region::options::CompactionOptions::Twcs(t) => {
